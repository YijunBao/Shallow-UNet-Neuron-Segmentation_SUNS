...
if __name__ == '__main__':
    ...
    list_Recall = np.zeros((num_CV, num_exp))
    list_Precision = np.zeros((num_CV, num_exp))
    list_F1 = np.zeros((num_CV, num_exp))
    list_time = np.zeros((num_CV, num_exp, 4))
    list_time_frame = np.zeros((num_CV, num_exp, 4))
    ...
    for (eid,Exp_ID) in enumerate(list_Exp_ID): #[::-1][1:]
        ...
        for CV in list_CV:
            # if CV==eid:
            #     continue
            print('Exp_ID={}, CV{}'.format(Exp_ID, CV))
            start = time.time()
            fff = get_unet() #size
            ...
            time_all = finish - start_network + time_pre
            ...
            savemat(dir_output+'Output_Masks_CV{}_{}.mat'.format(CV, Exp_ID), {'Masks_2':Masks_2, 'time_all': time_all})

            # %% Save information
            list_Recall[CV,eid] = Recall
            list_Precision[CV,eid] = Precision
            list_F1[CV,eid] = F1
            list_time[CV,eid] = np.array([time_pre, time_CNN, time_post, time_all])
            list_time_frame[CV,eid] = np.array([time_frame_pre, time_frame_CNN, time_frame_post, time_frame_all])

            del pmaps_b, prob_map#, Masks
