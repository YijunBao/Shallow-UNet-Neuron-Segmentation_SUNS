...
if __name__ == '__main__':
    ...
    list_time = np.zeros((num_CV, num_exp, 3))
    list_time_frame = np.zeros((num_CV, num_exp, 3))
    ...
    for (eid,Exp_ID) in enumerate(list_Exp_ID): #[::-1][1:]
        ...
        for CV in list_CV:
            ...
            savemat(dir_output+'Output_Masks_CV{}_{}.mat'.format(CV, Exp_ID), {'Masks_2':Masks_2, 'list_time_per':list_time_per[frames_initf:nframesf]})
            ...
            list_F1[CV,eid] = F1
            list_time[CV,eid] = np.array([time_init, time_online, time_all])
            list_time_frame[CV,eid] = np.array([time_frame_init, time_frame_online, time_frame_all])
