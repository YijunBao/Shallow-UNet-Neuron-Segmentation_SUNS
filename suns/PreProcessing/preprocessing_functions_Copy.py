...
from suns.PreProcessing.par1 import fastexp, fastmask, fastlog, \
    fastconv, fastquant, fastnormf, fastnormback, fastmediansubtract, fastmedian
...
def median_calculation_only(network_input, med_frame2=None, dims=None, median_decimate=1, display=True):
    '''Calculate the median (but not median-based standard deviation) of a video.
    ...
    if med_frame2 is None:
        med_frame2 = np.zeros(rows, cols, 1, dtype='float32')

    result = np.copy(network_input[::median_decimate, :rows, :cols].transpose([1, 2, 0]))
    fastmedian(result, med_frame2)
    # fastquant(result, np.array([0.5], dtype='float32'), med_frame2)
...
def median_calculation(network_input, med_frame2=None, dims=None, median_decimate=1, display=True):
