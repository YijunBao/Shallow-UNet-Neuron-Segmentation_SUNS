...
from suns.Online.functions_online import merge_2, merge_2_nocons, merge_complete, select_cons, select_cons_output, \
...
from suns.PostProcessing.complete_post import complete_segment_output
...
def suns_batch(dir_video, Exp_ID, filename_CNN, Params_pre, Params_post, dims, \
    ...
    Masks_2, times_cons = complete_segment_output(pmaps_b, Params_post_copy, display=display, p=p, useWT=useWT)
    if display:
        finish = time.time()
    ...
    return Masks, Masks_2, time_total, time_frame, times_cons
...
def suns_online(filename_video, filename_CNN, Params_pre, Params_post, dims, \
    ...
    for t in range(frames_initf,nframesf):
        ...
        if ((t - 3 - t_merge) == merge_every) or (t==nframesf-1):
            ...
            t_merge += merge_every
            if show_intermediate:
                Masks_2, times_cons = select_cons_output(tuple_temp)
    ...
    # convert to a 3D array of the segmented neurons
    Masks = np.reshape(Masks_2.toarray(), (Masks_2.shape[0], Lx, Ly)).astype('bool')
    return Masks, Masks_2, time_total, time_frame, list_time_per, times_cons
...
def suns_online_track(filename_video, filename_CNN, Params_pre, Params_post, dims, \
    ...
    Masks_cons, times_cons = select_cons_output(tuple_temp)
    # final result. Masks_2 is a 2D sparse matrix of the segmented neurons
    if len(Masks_cons):
