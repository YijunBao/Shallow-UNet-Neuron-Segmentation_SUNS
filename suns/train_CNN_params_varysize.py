...
def train_CNN(dir_img, dir_mask, file_CNN, list_Exp_ID_train, list_Exp_ID_val, \
    ...
    train_gen = data_gen_list(list_train_imgs, list_train_masks, batch_size=BATCH_SIZE, flips=True, rotate=True)
    if list_Exp_ID_val is not None:
        val_gen = data_gen_list(list_val_imgs, list_val_masks, batch_size=BATCH_SIZE, flips=False, rotate=False)
        ...
        for CV in list_CV:
            ...
            if os.path.exists(mat_filename) and load_exist: 
                ...
                Params_set_Mag = {'list_minArea': list(np.round(np.array(list_minArea) * Mag**2)), \
                    ...
                    'list_thresh_pmap': list_thresh_pmap, 'thresh_COM0': thresh_COM0 * Mag, \
                    'list_thresh_COM': list(np.array(list_thresh_COM) * Mag), \
                    'list_thresh_IOU': list_thresh_IOU, 'thresh_mask': thresh_mask, 'list_cons': list_cons}
                list_Recall, list_Precision, list_F1 = parameter_optimization_pipeline(
                    file_CNN, network_input, (Lx,Ly), Params_set_Mag, filename_GT, batch_size_eval, useWT=useWT, useMP=useMP, p=p)
