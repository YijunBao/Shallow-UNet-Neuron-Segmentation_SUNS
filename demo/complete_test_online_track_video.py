...
def main():
    ...
    dir_output = dir_parent + dir_sub + 'output_masks track video\\' #
    ...
    for CV in [0]: # list_CV: # 
        ...
        savemat(dir_output+'Output_Masks_{}.mat'.format(Exp_ID), {'Masks_2':Masks_2, \
            'list_time_per':list_time_per[frames_init:nframes], 'list_active_old':list_active_old})
        frame_save = 3000
        for n in range(int(np.ceil((nframes-frames_init)/frame_save))):
            savemat(dir_output+'Output_Masks_{}_{}.mat'.format(Exp_ID,n), \
                {'list_Masks_cons_2D_{}'.format(n):list_Masks_cons_2D[frame_save*n:frame_save*(n+1)]})
