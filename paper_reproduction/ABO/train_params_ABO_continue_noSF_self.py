...
if __name__ == '__main__':
    ...
    dir_output = dir_parent + 'output_masks self\\' # folder to save the optimized hyper-parameters
    dir_temp = dir_parent + 'temp self\\' # temporary folder to save the F1 with various hyper-parameters
    ...
    parameter_optimization_cross_validation("train_1_test_rest", list_Exp_ID, Params_set, \
        (rows, cols), (rowspad, colspad), dir_network_input, weights_path, dir_GTMasks, dir_temp, dir_output, \
        batch_size_eval, useWT=useWT, useMP=True, load_exist=True)
